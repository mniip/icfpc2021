...
separates (S2V2 a b) p q = (signedArea ba (p .-. a) <= 0) `xor` (signedArea ba (q .-. a) <= 0)
  where !ba = b .-. a

separatesStrictly :: S2 -> V2 -> V2 -> Bool
separatesStrictly (S2V2 a b) p q = signedArea ba (p .-. a) * signedArea ba (q .-. a) < 0 -- ?case analysis for overflow?
  where !ba = b .-. a
